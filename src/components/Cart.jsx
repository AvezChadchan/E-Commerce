import useItemStore from '../store/itemStore'
import { IoMdRemoveCircleOutline } from "react-icons/io";

const Cart = () => {
    const { cart, removeFromCart } = useItemStore();
    const subtotal = cart.reduce((acc, item) => acc + item.price, 0)
    return (
        <>
            <div className='px-8 py-6 flex justify-between items-start'>
                {cart.length === 0 && <p className='font-bold text-2xl text-yellow-500 fixed top-70 left-150'>Cart is Empty</p>}
                <div className='flex flex-col gap-5 w-[65%] my-6 '>
                    <h2 className='text-2xl font-bold text-black mt-10'>
                        Shopping Cart</h2>
                    {cart.map((i) => (
                        <div className='flex gap-5 w-[90%] shadow-md shadow-black rounded-2xl hover:shadow-2xl transition justify-around bg-gray-50 p-2'>
                            <img src={i.image} alt="" className='w-30 rounded-2xl p-1 ' />
                            <div className='flex flex-col'>
                                <h2 className='font-semibold mt-5'>{i.name}</h2>
                                <p className='text-gray-500'>${i.price}</p>
                            </div>
                            <button onClick={() => {
                                removeFromCart(i);
                                alert(`${i.name} is removed from Cart`)
                            }} className='flex gap-2 h-15 p-3 my-5 bg-yellow-400 rounded-2xl hover:bg-yellow-500 text-white text-xl font-bold'>Remove<IoMdRemoveCircleOutline size={25} className='my-1' /></button>
                        </div>
                    ))}
                </div>
                {subtotal > 0 && (
                    <div className='payment fixed top-20 right-10 w-[30%] h-auto p-6 bg-gray-50 rounded-2xl shadow-lg shadow-black text-center mt-50'>
                        <div className="subtotal mb-4">
                            <h2 className='text-xl font-bold'>Subtotal: </h2>
                            <p className='text-lg'>${subtotal}</p>
                        </div>
                        <button className='font-bold bg-yellow-400 text-white text-xl w-full rounded-xl hover:bg-yellow-500'>Proceed to Checkout</button>
                    </div>
                )}

            </div>
        </>
    )
}

export default Cart
